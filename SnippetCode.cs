...
public static class Snippet
{
    ...
    public static float XAbsBiggerValue(this Vector3 vec) => Mathf.Abs(vec.x) > Mathf.Abs(vec.y) && Mathf.Abs(vec.x) > Mathf.Abs(vec.z) ? vec.x : float.MinValue;

    public static float YAbsBiggerValue(this Vector3 vec) => Mathf.Abs(vec.y) > Mathf.Abs(vec.x) && Mathf.Abs(vec.y) > Mathf.Abs(vec.z) ? vec.y : float.MinValue;

    public static float ZAbsBiggerValue(this Vector3 vec) => Mathf.Abs(vec.z) > Mathf.Abs(vec.x) && Mathf.Abs(vec.z) > Mathf.Abs(vec.y) ? vec.z : float.MinValue;
    ...
    /// <returns></returns>
    public static ConfigurableJoint CreateJointToProjectileForCreatureAttraction(this Item projectile, RagdollPart attractedRagdollPart)
    {
    ...
    public static void SetTKHandle(this Item item, bool active)
    {
        ...
        {
            if (!active && item.handles[i].telekinesisHandler)
                item.handles[i].telekinesisHandler.telekinesis.TryRelease();
    ...
    public static Vector3 RotatePointAroundPivot(this Vector3 point, Vector3 pivot, Vector3 axisOfPivot, float degreesOfRotation)
    {
    ...
    public static Item CenteredItemInConeRadius(this Vector3 position, float radius, Vector3 directionOfCone, float angleOfCone, bool ignoreFlyingItem = true, bool ignoreThrownItem = true, bool ignoreKinematicItem = true, bool ignoreHand = true, Item itemToExclude = null)
    {
        ...
        foreach (Collider collider in colliders)
        {
            if (collider.attachedRigidbody?.GetComponent<CollisionHandler>()?.item is Item item)
            {
                ...
                && (ignoreFlyingItem ? true : item.isFlying)
                && (ignoreThrownItem ? true : item.isThrowed)
                && (ignoreKinematicItem ? !item.physicBody.isKinematic : true)
    ...
    public static GameObject AddZoneToGameObject(this Transform transform, float radius, bool useDebug = false, bool colliderIsTrigger = true, LayerName layerName = LayerName.ItemAndRagdollOnly, string shaderName = "Sprites/Default", string physicMaterialString = "Flesh")
    {
        GameObject zone;
